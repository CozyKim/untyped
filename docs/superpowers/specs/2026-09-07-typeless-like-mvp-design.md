...
1. ~~클립보드 삽입과 오버레이 공존~~ — **해소됨.** 세 앱에서 삽입·복원·오버레이
   공존을 모두 확인했다. 복원 지연은 150ms로 확정(8절).
2. ~~오른쪽 `⌥` 단독 키 수신~~ — **해소됨.** 눌림·뗌이 1:1로 잡히고, 왼쪽 `⌥`는
   걸러지며, 누른 채 타이핑해도 방해되지 않는다(11절).
...
로컬 자체 서명 인증서(`TypelessLike Local Dev`, codeSigning 전용)로 서명한다.
`Scripts/bundle.sh`가 키체인에서 그 신원을 찾아 쓰고, 없으면 ad-hoc으로 떨어지되
경고를 출력한다.

ad-hoc을 쓰지 않는 이유는 designated requirement가 달라지기 때문이다.

```
ad-hoc      designated => cdhash H"..."                        빌드마다 바뀜
자체 인증서   designated => identifier "com.jaehyun.typelesslike"
                          and certificate root = H"..."         고정
```

손쉬운 사용 권한은 designated requirement를 키로 하므로, ad-hoc이면 재빌드할
때마다 권한이 무효화되어 매번 다시 승인해야 한다. 자체 인증서로 서명하면 한 번만
승인하면 된다. 인증서는 신뢰 설정이 없어도 된다 — `codesign`은 신뢰되지 않은
자체 서명 인증서도 사용한다.
...
few-shot 전체를 빼면 정정 처리가 **반대로** 뒤집힌다. 같은 서버·같은 모델에
`temperature: 0`으로 각각 5회씩 보낸 대조다.

```
입력: 어 내일 아침에 보낼게요 아니 오늘 저녁에 보낼게요

few-shot 없음  5/5  "내일 아침에 보낼게요"      정정 이전을 남기고 이후를 버림
few-shot 있음  5/5  "오늘 저녁에 보낼게요."     규칙대로
```

양쪽 모두 5회가 일치하므로 모델은 이 입력에서 결정적이고, 차이는 예시의 유무
하나다. 규칙 1의 문장만으로는 부족하다 — 모델이 그 지시를 따르게 만드는 것은
예시다.
...
TextEdit, 브라우저 주소창, 서드파티 앱에서 50·150·400ms를 교차 시험해 세 값 모두
실패가 없었다. 가장 작은 값 대신 150ms를 쓰는 이유는 실패 모드가 조용하기
때문이다 — 너무 짧으면 붙여넣기가 복원된 옛 클립보드 내용을 집어가고, 사용자는
말하지 않은 문장이 삽입된 것을 나중에야 안다. 지연을 늘려도 붙여넣기 자체는
즉시 일어나므로 체감 비용이 없다.

같은 시험에서 9절 설정의 `NSPanel`이 떠 있어도 삽입이 정상임을 확인했다.
오버레이는 포커스를 빼앗지 않는다.
...
이 조합은 삽입 스파이크에서 함께 검증했다. 세 앱에서 패널이 떠 있는 채로
붙여넣기가 정상 동작했다.
...
스파이크로 확인했다. `flagsChanged`가 눌림과 뗌을 모두 보내고, keyCode 61로
왼쪽 `⌥`(keyCode 58)를 걸러낼 수 있으며, `addGlobalMonitorForEvents`는 이벤트를
소비하지 않으므로 누른 채 타이핑해도 방해가 없다.

MVP에서 키는 코드 상수로 두고 변경 UI는 만들지 않는다.
...
- `TextRefiner` 폴백 — 항상 throw하는 스텁으로 Coordinator가 원본을 삽입하는지
  검증한다.
- `RefinementPrompt` — 규칙 번호가 정확히 1~4이고(줄 앞 번호로 판정하여 `5)` 같은
  변형과 누락·중복까지 잡는다), few-shot 여덟 쌍의 내용과 순서가 보존되며,
  시스템 프롬프트 끝의 nonce가 호출마다 달라지는지 검증한다. nonce는 1000개 표본의
  유일성으로 확인한다 — 두 호출을 비교하는 것만으로는 엔트로피가 낮은 nonce를
  놓치고, 그 경우 3절의 캐시 결함으로 엉뚱한 문장이 삽입된다.
