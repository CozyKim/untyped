...
규칙 3 (용어 복원): 아래 용어 사전에 있는 단어가 한글로 음차돼 있으면 사전의 영문
표기로 되돌린다. 발음이 뭉개져 있어도 사전 안에서 가장 가까운 것을 고른다.
  사전: <TERM_GLOSSARY>
  사전에 없는 말은 절대 영어로 바꾸지 않는다. 한글 그대로 둔다. 추측해서 지어내지
  않는다.
규칙 4 (형식): 문장부호와 띄어쓰기를 정리한다.
규칙 5 (보존): 정정되지 않은 내용은 하나도 빠뜨리지 않는다.
다듬은 문장만 출력한다.
```

`<TERM_GLOSSARY>`는 `RefinementPrompt`가 소유하는 개발·IT 용어 목록이다. MVP에서는
코드 상수로 두고 변경 UI를 만들지 않는다.

사전 마지막 줄의 금지 조항이 이 규칙의 핵심이다. 빼면 모델이 뭉개진 발음을 추측해
틀린 단어를 지어낸다(3절).
...
| 5 | 음차된 개발 용어를 영문으로 되돌리는 경우 |
| 6 | 영어 용어가 없는 순수 한국어 — 손대지 않는다 |

3번과 4번이 없으면 모델은 짧은 문장만 편집하거나, 정정 시 앞부분 내용을 통째로
날린다. 6번이 없으면 일상 한국어까지 영문으로 바꾼다. 모두 측정에서 관찰했다.
...
- 용어 복원 과정에서 원래 단어를 지워버리는 경우가 관찰됐다. 재발하면 규칙 5를
  강화한다.
